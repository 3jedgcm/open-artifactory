import { Button, Card, CardBody, CardFooter, CardHeader, Container, FormControl, FormErrorMessage, FormLabel, Heading, HStack, Input, Link, Spinner, Text, VStack } from '@chakra-ui/react'
...
export default function Authentication() {
  ...
  const initialisation = async () => {
    try {
      let result = await fetch(BASE_URL + "/init")
      ...
      <Card  >
        ...
        </CardHeader>
        {
          otp.length > 0 && otpSecret.length > 0 ?
            <CardBody>
              <Text textAlign='center' fontWeight='bold' color='blackAlpha.700' mt={1} >Add configuration to your favorite application</Text>
              <HStack justifyContent='center' marginTop={4}>
                <QRCode onClick={() => initialisation()} cursor='pointer' value={otp} size={198} />
              </HStack>
              <FormControl isInvalid={error} mt='4' >
                <FormLabel fontWeight='bold' color='blackAlpha.700'>Confirm your one time password :</FormLabel>
                <HStack>
                  <Input borderWidth={2} fontWeight='bold' color='blackAlpha.800' errorBorderColor='red.500' placeholder='000000' disabled={loading} value={code} maxLength={6} onChange={(event) => { setCode(event.currentTarget.value); confirmCode(event.currentTarget.value) }} />
              ...
              {
                internalError ?
                  <Text fontWeight='bold' marginTop={2} color='red.500'>Internal error server</Text> :
                  null
              }
            </CardBody>
            :
            <CardBody>
              <FormControl isInvalid={error} mt='4' >
                <FormLabel fontWeight='bold' color='blackAlpha.700'>Enter your one time password code :</FormLabel>
                <HStack>
