...
export default function UploadFile({
  ...
  return (
    <Modal isOpen={upload} onClose={() => {
      uploadLoading == false && setUpload(false)
    }}>
      <ModalOverlay/>
      <ModalContent marginLeft={4} marginRight={4}>
        <ModalHeader>
          <Text fontSize="2xl" textAlign="center" fontWeight="bold"
                color="blackAlpha.700">{'Upload artifactory'}</Text>
        </ModalHeader>
        <Divider/>
        <ModalCloseButton/>
        <ModalBody mt={2}>
            ...
            uploadLoading ?
              <VStack justifyContent={'center'} margin={4}>
                <Text fontSize="lg" textAlign="center" fontWeight="bold"
                      color="blackAlpha.700">{percent + '%'} </Text>
                <Spinner size="md"/>
              </VStack>
              :
              <Dropzone uploadLoading={uploadLoading} onDropFile={(file) => {
                uploadArtifact(file)
              }}/>
          }
        </ModalBody>
